# Copyright 2008 by Roy Wright
# License: GPL version 2 (http://www.opensource.org/licenses/gpl-2.0.php)
# generated by mysql2models.rb 0.2 Tue Jun 17 01:17:05 -0500 2008
#
# == Synopsis
# Database Table: pluto_main.DeviceTemplate_MediaType
# Through Join Table
#
# * primary keys: PK_DeviceTemplate_MediaType
# * foreign keys: FK_DeviceTemplate,FK_MediaType
# * external keys: 
#
# == References
# * PlutoMain::LmceDeviceTemplate
# * PlutoMain::LmceMediaType
#
# == Referenced By
# * PlutoMain::LmceDeviceTemplateMediaTypeDesignObj
#
# == Database Table Columns
#   * PK_DeviceTemplate_MediaType    [Type:int(11), Default:NULL, Key:PRI, Null:NO, Extra:auto_increment]
#   * FK_DeviceTemplate              [Type:int(11), Default:0, Null:NO]
#   * FK_MediaType                   [Type:int(11), Default:0, Null:NO]
#   * CanPlayFromDiskDrive           [Type:tinyint(1), Default:0, Null:NO]
#   * CanStoreOnServer               [Type:tinyint(1), Default:0, Null:NO]
#   * CanSetPosition                 [Type:tinyint(1), Default:0, Null:NO]
#   * CanPlayInMultipleAreas         [Type:tinyint(1), Default:0, Null:NO]
#   * StopOtherMediaInEntArea        [Type:tinyint(1), Default:0, Null:NO]
#   * Extensions                     [Type:varchar(40), Default:NULL, Null:YES]
#   * psc_id                         [Type:int(11), Default:NULL, Key:UNI, Null:YES]
#   * psc_batch                      [Type:int(11), Default:NULL, Null:YES]
#   * psc_user                       [Type:int(11), Default:NULL, Null:YES]
#   * psc_frozen                     [Type:tinyint(1), Default:0, Null:YES]
#   * psc_mod                        [Type:timestamp, Default:CURRENT_TIMESTAMP, Null:YES]
#   * psc_restrict                   [Type:int(11), Default:NULL, Null:YES]
#
# == Associations
# * belongs_to lmce_device_template
# * belongs_to lmce_media_type
# * has_many lmce_device_template_media_type_design_objs
#

class PlutoMain::LmceDeviceTemplateMediaType < PlutoMainDatabase
  set_table_name "DeviceTemplate_MediaType"
  set_primary_key "PK_DeviceTemplate_MediaType"

  # foreign key references
  # FK_DeviceTemplate_MediaType references PlutoMain::LmceDeviceTemplate (FK_DeviceTemplate)
  belongs_to  :lmce_device_template,
      :class_name => "PlutoMain::LmceDeviceTemplate",
      :foreign_key => "FK_DeviceTemplate"

  # FK_DeviceTemplate_MediaType references PlutoMain::LmceMediaType (FK_MediaType)
  belongs_to  :lmce_media_type,
      :class_name => "PlutoMain::LmceMediaType",
      :foreign_key => "FK_MediaType"

  # 1:n  (FK_DeviceTemplate_MediaType)
  has_many  :lmce_device_template_media_type_design_objs,
      :class_name => "PlutoMain::LmceDeviceTemplateMediaTypeDesignObj",
      :foreign_key => "FK_DeviceTemplate_MediaType"

  # handle psc_mod timestamping

# Set the psc_mod timestamp to the current time when creating a record.
  def before_create
    self.psc_mod ||= Time.now
  end

# Set the psc_mod timestamp to the current time when updating a record.
  def before_update
    self.psc_mod = Time.now
  end

  # add your model extensions after this line

end

